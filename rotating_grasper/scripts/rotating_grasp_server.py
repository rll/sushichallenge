...
class RotatingGraspServer():
    ...
    def handle_plate_push(self,req):
        command = req.command
        print 'received command:'
        print command
        ...
        with CartController(self.cmclient):
            ...
            push_offset_init = -0.04
            push_offset = push_offset_init
            push_offset_final = command.push_dist
            ...
            while True:
                ...
                if has_been_past_perigee:
                    if start_drop_angle <= now_angle and now_angle < start_grip_angle:
                        ...
                        print 'starting push'
                        offset = 0
                        push_offset = (push_offset_final - push_offset_init) * (now_angle - start_grip_angle) / (math.pi - start_grip_angle) + push_offset_init
                        ...
                        if finish_lift_angle <= now_angle:
                            #TODO: determine success
                            print 'push finished'
                            return RotatingPlatePusherResponse(True)
                        offset = offset_init * (now_angle - math.pi) / (finish_lift_angle - math.pi)
                        push_offset = push_offset_final
                ...
                offset_vertical   = offset * math.cos(outward_angle)
                offset_horizontal = offset * math.sin(outward_angle) + push_offset

                q1 = quaternion_about_axis(math.pi/2,(0,1,0))
                #q2 = quaternion_about_axis(math.pi/2+command_angle,(1,0,0))
                q2 = quaternion_about_axis(command_angle,(1,0,0))
                q3 = quaternion_about_axis(-outward_angle,(0,0,1))
                q12 = quaternion_multiply(q1,q2)
                q = quaternion_multiply(q12,q3)
                pointing_axis = quaternion_matrix(q)[:3,0]
    ...
    def __init__(self):
        ...
        self.s2 = rospy.Service('rotating_plate_pusher', RotatingPlatePusher, self.handle_plate_push)
