...
class RotatingGraspServer():
    def handle_rotating_grasp(self,req):
        ...
        with CartController(self.cmclient):
            ...
            half_height = command.half_height
            ...
            # .2 for tool frame, .08 for ???
            offset_init = rospy.get_param('offset_init',.2 + .08 + math.sqrt(x_diff**2 + y_diff**2 + half_height**2))
            offset = offset_init
            ...
            target.pose.position.z = command.center.z + half_height + offset_vertical
