...
class TrajectoryLibrary(object):
    ...
    #         
    #     traj_len = len(array)
    #     traj = np.zeros(shape=(traj_len,), dtype=TrajectoryPoint)
    #     traj["xyz_l"] = array[:, 0:3]
    #     traj["quat_l"] = array[:, 3:7]
    #     traj["grip_l"] = array[:, 7]
    #     traj["grab_l"] = array[:, 8]
    # 
    #     traj["xyz_r"] = array[:, 9:12]
    #     traj["quat_r"] = array[:,12:16]
    #     traj["grip_r"] = array[:, 16]
    #     traj["grab_r"] = array[:, 17]
    #     
    #     traj["rope"] = array[:, 18:].reshape(traj_len, N_CTRL_PTS, 3)
    # 
    #     self.root["demos"].create_dataset(name, data=traj)
