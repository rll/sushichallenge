...
def get_base_positions(xyzs_world):
    ...
    invreach = ndi.binary_erosion(invreach,np.ones((5,5,5)))
    
    
    xticksir = - f["xticks"][::-1]
    yticksir = - f["yticks"][::-1]
    
    xgrabmax, ygrabmax, _ = np.max(xyzs_world,axis=0)
    xgrabmin, ygrabmin, _ = np.min(xyzs_world, axis=0)

    xmin = xgrabmin + xticksir[0]
    xmax = xgrabmax + xticksir[-1]
    ymin = ygrabmin + yticksir[0]
    ymax = ygrabmax + yticksir[-1]
    
    nxsmall,nysmall,_ = invreach.shape
    
    xticks = np.arange(xmin-DL, xmax+DL, DL)
    yticks = np.arange(ymin-DL, ymax+DL, DL)
    
    bigshape = (xticks.size, yticks.size)
    
    def make_shifted_image(xtarg, ytarg, ztarg): # put origin at x,y,z

        xllind = intround(   (xtarg + xticksir[0] - xticks[0])/DL   )
        yllind = intround(   (ytarg + yticksir[0] - yticks[0])/DL   )
        zind = intround(   (ztarg - ztorso - f["zticks"][0])/DL   )
        newim = np.zeros(bigshape, 'uint8')
        newim[xllind:(xllind+nxsmall),yllind:(yllind+nysmall)] = invreach[:,:,zind]>0
        return newim
    
    stack = np.zeros((len(xyzs_world), xticks.size, yticks.size),'uint8')
    for (i,(x, y, z)) in enumerate(xyzs_world):
        stack[i] = make_shifted_image(x,y,z)

    #xindstart = intround(   (x - f["xticks"][0])/DL   )
    #yindstart = intround(   (y - f["yticks"][0])/DL   )

    DIST_COST = 1.
    INFEAS_COST = 1000.

    costs = DIST_COST + (stack==0)*INFEAS_COST
    ...
    starts = [(0,i,j) for (i,j) in np.ndindex(xticks.size, yticks.size)]
    ends = [(stack.shape[0]-1,i,j) for (i,j) in np.ndindex(xticks.size, yticks.size)]    
    ...
    triples = mcp_obj.traceback((len(xyzs_world)-1, ix, iy))    
    
    out = []
    for (it, ix, iy) in triples:
        out.append((xticks[ix]-xtorso, yticks[iy]-ytorso))
...
def get_base_positions2(xyzs_world):
    ...
    return np.c_[xticks[xinds_base] - x_torso, yticks[yinds_base] - y_torso]
